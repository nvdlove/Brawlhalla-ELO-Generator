        ...
        private async Task<bool> IsReconnectPopupVisible()
        {
            try
            {
                ...
                using (Bitmap screenshot = CaptureWindow(hwnd))
                {
                    ...
                    string debugFile = $"debug_reconnect_{DateTime.Now:HHmmss}.png";
                    screenshot.Save(debugFile, ImageFormat.Png);
                    Log($"   • Debug screenshot saved: {debugFile}");
                        ...
                        if (File.Exists(reconnectTemplatePath))
                        {
                            using (Mat template = Cv2.ImRead(reconnectTemplatePath, ImreadModes.Grayscale))
                            {
                                if (!template.Empty())
                                {
                                    ...
                                    Log($"   • Template match score: {bestScore:F3}");
                                    ...
                                    if (bestScore > 0.6)
                                    {
                                        if (bestScore > 0.8)
                                            ...
                                            Log($"   ⚠️  Reconnect popup detected (WEAK match: {bestScore:F3})");

                                        return true;
                                    }
                                    else
                                    {
                                        Log($"   • No popup detected (score: {bestScore:F3})");
                                    }
                                }
                            }
                        }

                        Log($"   • No template match found");
                        return false;
                    }
                }
            }
        ...
        private async Task NavigateToQueue()
        {
            ...
            try
            {
                ...
                if (hasReconnect)
                {
                    ...
                    {
                        Log("   • WARNING: Popup might still be there, but not pressing ESC again");
